...
#            for m in w7:
#                for n in w7:
#                    total += final_outImagedata7[k][i + m][j + n] * final_outImagedata7[k][i + m][j + n]
#            
#            feature[k][i][j] = total/49    
            
for k in range(25):
    l = 0
    for i in range(height):
        for j in range(width):
            feature[l][k] = np.sum(abs(final_outImagedata21[k][i:i+21,j:j+21]))/441
            l+=1
...
#K-means clustering
kmn = KMeans(n_clusters=7).fit(feature_norm)
labels = kmn.labels_
...
#cv2.imwrite('segmentation15.jpg', output) 
#cv2.imwrite('segmentation17.jpg', output) 
#cv2.imwrite('segmentation19.jpg', output) 
cv2.imwrite('segmentation21.jpg', output) 
#cv2.imwrite('segmentation31.jpg', output) 
...
#cv2.imwrite('segmentation15.jpg', output) 
#cv2.imwrite('segmentation17_std.jpg', output) 
#cv2.imwrite('segmentation19_std.jpg', output) 
#cv2.imwrite('segmentation21_std.jpg', output) 

#kernel = np.ones((5,5), np.float32)
#A = cv2.morphologyEx(output, cv2.MORPH_CLOSE, kernel)
#cv2.imwrite('segmentation17.jpg', A) 
