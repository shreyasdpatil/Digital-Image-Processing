function [F, Recall_gt,sumR,Precision_gt,sumP, threshold] = edgesEvalImg( E, G, varargin )
...
for k = 1:K
  ...
  Recall_gt(k) = cntR(k)./sumR(k);
  % compute precision (edges can match any gt image)
  cntP(k) = nnz(matchE); sumP(k) = nnz(E1);
  Precision_gt(k) = cntP(k)./sumP(k);
  F(k) = 2*Precision_gt(k)*Recall_gt(k)/(Precision_gt(k) + Recall_gt(k));
  threshold(k) = k;
