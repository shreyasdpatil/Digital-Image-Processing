...

{
    ...
    float newImagedata[height+4][width+4][1];
    ...
    for (int i = 2; i<height+2; i++){
        if ((i-2)%2!=0) {
            for (int j = 2; j < width+2; j++) {
                ...
                float error = (newImagedata[i][j][0] - outImagedata[i-2][j-2]);
                newImagedata[i][j+1][0] += (7/48.0) * error;
                newImagedata[i][j + 2][0] += (5/48.0) * error;

                newImagedata[i+1][j-2][0] += (3/48.0) * error;
                newImagedata[i+1][j-1][0] += (5/48.0)*error;
                newImagedata[i+1][j][0] += (7/48.0)*error;
                newImagedata[i+1][j+1][0] += (5/48.0)*error;
                newImagedata[i+1][j+2][0] += (3/48.0)*error;

                newImagedata[i+2][j-2][0] += (1/48.0)*error;
                newImagedata[i+2][j-1][0] += (3/48.0)*error;
                newImagedata[i+2][j][0] += (5/48.0)*error;
                newImagedata[i+2][j+1][0] += (3/48.0)*error;
                newImagedata[i+2][j+2][0] += (1/48.0)*error;
            }
        }
